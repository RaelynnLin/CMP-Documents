...
| 項目 | 說明 |
|------|------|
| **根因** | 訂單表單欄位（Account ID／Partner ID／網域／訂閱 ID／專案名稱、客戶編號等）於送出前未對字串值做去前後空格處理，使用者複製貼上時夾帶的前後空格會原樣送至後端，造成後端比對、查詢或建單時資料不一致。 |
| **解決方法** | 採雙層 trim 設計：①**onFormChange 通用 trim**——各品牌 extra-field 與共用表單（order-header／order-customer／sub-order）於輸入變更時，對 `typeof string` 之值即時 trim 後寫入 model（僅改 model 值、不改寫畫面顯示）；②**送出前白名單 trim**——`addOrder`／`saveOrder`／`createModifyOrder` 送出前，依 `TRIM_FIELD_PATHS` 白名單（含 `cloudId`、`originalInfo.partnerId`、`extra.domain`、`contractIds[]` 等巢狀／陣列路徑）逐一 trim，作為保證點以涵蓋未觸發 onFormChange 的載入值。trim 僅作用於字串、僅去前後空格（保留中間空格），非字串欄位（陣列／日期／數字）以型別守衛排除。 |

> 詳細受測檔案與白名單欄位見〈附錄 A〉〈附錄 B〉。
