...
原 handler 以 `this.notify.error('[add-order]', err)` 將整個 `err` **物件**當作通知「內容」傳入；NG-ZORRO 通知元件會對該參數做字串化，物件即被轉成 `[object Object]`，後端真正回傳的訊息因此被吞掉。

**根因 2 — handler 自身在無 `code` 的錯誤上拋 TypeError**

handler 第一行 `if (err.code.indexOf('bpm_fail') !== -1)` 假設 `err.code` 必為字串。但 `postOrderWithFullError()` 的 `catchError` 有兩條丟出路徑（[order.service.ts:70-79](../../cmpweb_T100/src/app/share/services/order.service.ts#L70-L79)）：

| 後端回應 | 丟出內容 | `err.code` |
|---|---|---|
| `error.error?.info.code` 存在（如 bpm 流程錯誤） | `throwError(() => error.error.info)`（物件，含 `code`/`message`）| 有值（字串）|
| 無 `info.code`（**附件數量超限即屬此類**）| `throwError(() => new Error(errorMessage))`（標準 `Error`，無 `code`）| `undefined` |
...
2. **顯示後端訊息**：將 `notify.error('[add-order]', err)` 改為取 `err?.message || err?.error?.message || this.translate.instant('error message')` 作為通知**內容文字**，避免物件被字串化成 `[object Object]`，使後端的數量超限訊息能正確呈現。
